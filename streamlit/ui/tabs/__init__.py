...
from . import auth, uploads, tab_plugins, inference, workflows, health, broadcast

def render_all_tabs(base_url: str, feats: dict) -> None:
    st.title("NeuroNexus-ai Dashboard (Multi-Server)")
    st.caption("Professional interface for managing multiple FastAPI servers: Add/Edit/Delete · Independent Tokens · Request Broadcasting")

    tabs = st.tabs(["Auth", "Uploads", "Plugins", "Inference", "Workflows", "Health/Info", "Broadcast"])
    auth.render(tabs[0], base_url, feats)
